	...
	if vizconnect.isPendingInit('event', _name, initFlag, initList):
		...
		if initFlag&vizconnect.INIT_MAPPINGS:
			...
			if initFlag&vizconnect.INIT_MAPPINGS_PER_FRAME:
				...
				#VC: set the update function which checks for input signals
				def update(event):
					if rawInput['r_hand_input'].isButtonDown(2) and\
						rawInput['r_hand_input'].isButtonDown(0):# make=HTC, model=Vive Controller, name=r_hand_input, signal=Button Trigger (along with) make=HTC, model=Vive Controller, name=r_hand_input, signal=Button Menu
						event.sendOnce(e=viz.Event(mag=1 * 1))
